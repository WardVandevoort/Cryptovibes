...
fetch("https://cryptovibes.herokuapp.com/api/v1/cryptovibes/transfer"
     ...
     json.data.transactions.forEach(transaction => {
         ...
         if (transaction.receiver_id == tokenData) {
             ...
             <p><img class="avatar" src="../public/minilogo.svg" /> ${transaction.sender_id} <span class="payment__quantity">+${transaction.quantity}</span></p>
             </div>`;
             ...
             <p><img class="avatar" src="../public/minilogo.svg" /> ${transaction.receiver_id} <span class="payment__quantity">-${transaction.quantity}</span></p>
             </div>`;
             document.querySelector(".history").insertAdjacentHTML('afterend', payment);
          }
